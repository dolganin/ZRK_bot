...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if await is_user_registered(user_id):
        await send_template(bot, message, "start_already_registered", reply_markup=main_menu())
        return
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await send_template(bot, message, "reg_ask_course", reply_markup=keyboard)
...
async def process_course(message: types.Message, state: FSMContext):
    ...
    await send_template(bot, message, "reg_ask_faculty", reply_markup=keyboard)
...
async def unknown_command(message: types.Message):
    await send_template(bot, message, "unknown_command", reply_markup=main_menu())
